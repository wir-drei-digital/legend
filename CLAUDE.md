...
- **Agent messaging (signal bus):** `Legend.Core.Signals` domain — pairwise `Message` (inbox = `read_at IS NULL`, `from_session_id` nil = the human). Agents call five MCP tools (`send_message`/`read_messages`/`start_agent`/`handoff`/`list_agents`) in `Legend.Core.Signals.Tools` via a hand-rolled JSON-RPC endpoint at `POST /api/mcp` (first router scope, NO MCP library). Auth is a per-session bearer token (`Session.mcp_token`) that *also* identifies the caller — agents never assert their own identity. `inbox:<id>` PubSub drives the SessionServer PTY nudge (the ONLY runtime PTY injection — a single debounced line, `config :legend, :nudge_debounce_ms` default 2000, with the submit CR written separately after `:nudge_submit_delay_ms` (default 150) because ink TUIs like Claude Code treat text+CR in one chunk as a paste and never submit; bodies are pulled via `read_messages`, never injected); `signals` topic drives the `signals:timeline` channel. Spawned sessions carry `spawned_by_session_id` (delegation lineage) + `instructions` (delivered as the CLI's initial prompt). MCP registration is per-harness: Claude Code per launch (`--mcp-config` + `--allowed-tools mcp__legend`); Hermes via a one-time `mcp_servers.legend` entry in `~/.hermes/config.yaml` using `${LEGEND_MCP_URL}`/`${LEGEND_SESSION_TOKEN}` placeholders — applied consent-gated through the harness setup seam (optional `setup/0`/`apply_setup/0` callbacks, `GET /api/harnesses` setup field, `POST /api/harnesses/:id/setup`; UI in the new-session dialog + `/settings`). See `docs/superpowers/specs/2026-06-12-agent-messaging-design.md`.
...
- `just dev`/`dev-desktop` repeatedly crashing with `could not compile application: legend ... config/config.exs ... changed` is a Phoenix 1.8.7 dev-reloader false positive, not a real config error: editing `config.exs`/`runtime.exs` in a *runtime-only* way bumps its mtime past the `_build/.../compile.elixir` manifest, but Elixir 1.20's fine-grained tracking recompiles nothing, so the manifest mtime never catches up and the reloader's crude mtime check raises on every request. Unstick with `cd backend && mix compile --force` (a plain restart won't — it doesn't force recompilation). Dev-only; prod/Burrito releases compile fresh.
