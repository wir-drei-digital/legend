...
- **Agent messaging (signal bus):** `Legend.Core.Signals` domain — pairwise `Message` (inbox = `read_at IS NULL`, `from_session_id` nil = the human). Agents call five MCP tools (`send_message`/`read_messages`/`start_agent`/`handoff`/`list_agents`) in `Legend.Core.Signals.Tools` via a hand-rolled JSON-RPC endpoint at `POST /api/mcp` (first router scope, NO MCP library). Auth is a per-session bearer token (`Session.mcp_token`) that *also* identifies the caller — agents never assert their own identity. `inbox:<id>` PubSub drives the SessionServer PTY nudge (the ONLY runtime PTY injection — a single debounced line, `config :legend, :nudge_debounce_ms` default 2000; bodies are pulled via `read_messages`, never injected); `signals` topic drives the `signals:timeline` channel. Spawned sessions carry `spawned_by_session_id` (delegation lineage) + `instructions` (delivered as the CLI's initial prompt). MCP registration is per-harness: Claude Code per launch (`--mcp-config` + `--allowed-tools mcp__legend`); Hermes has no per-launch flag — a one-time `mcp_servers.legend` entry in `~/.hermes/config.yaml` using `${LEGEND_MCP_URL}`/`${LEGEND_SESSION_TOKEN}` placeholders (Hermes interpolates env per process; without it Hermes runs but never sees the tools). See `docs/superpowers/specs/2026-06-12-agent-messaging-design.md`.
