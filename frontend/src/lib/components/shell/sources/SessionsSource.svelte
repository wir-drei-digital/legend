<script lang="ts">
	...
	// A session is "placed" when it's tiled in the active space — either as a
	// derived session tile (layout.has) or via a manual session binding.
	function isPlaced(id: string): boolean {
		return (
			workspaceStore.active.layout.has(id) ||
			workspaceStore.active.layout.tiles.some((t) => {
				const b = workspaceStore.binding(t);
				return b?.kind === 'session' && b.params.sessionId === id;
			})
		);
	}
	...
	const groups = $derived.by((): Group[] => {
		const needs = rows.filter((r) => r.state.attention);
		const running = rows.filter((r) => r.state.kind === 'running' && !r.state.attention);
		const idle = rows.filter(
			(r) => (r.state.kind === 'idle' || r.state.kind === 'done') && !r.state.attention
		);
		return [
