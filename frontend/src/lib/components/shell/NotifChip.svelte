...
>
	...
	<span class="font-mono text-meta text-ink-1">{label}</span>
	{#if detail}
		<span class="max-w-[120px] truncate font-mono text-meta text-ink-3">· {detail}</span>
	{/if}
	<span class="font-mono text-micro font-bold tracking-[0.08em]" style:color={flagColor}>{flag}</span>
</button>
