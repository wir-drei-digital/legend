...
Release builds must go through `backend/scripts/build-release.sh [legend|legend_desktop]` — a Codex hook blocks raw `MIX_ENV=prod mix` command strings, and the script also auto-provisions the zig 0.15.2 that Burrito 1.5 requires (installed zig 0.16 is too new).
