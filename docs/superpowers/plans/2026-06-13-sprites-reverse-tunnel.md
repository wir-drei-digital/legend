...
- `backend/lib/legend/tunnels/sprite_proxy/server.ex` — `Legend.Tunnels.SpriteProxy.Server`: per-tunnel GenServer owning carrier + mux + streams + splice. (Mid-session carrier reconnect-on-drop is deferred to live bring-up — see the spec's status note.)
...
- **Spec coverage:** behaviour+registry (Task 2) ✓; sprites client (Task 4) ✓ *(exec/fs shapes UNVERIFIED — token-blocked)*; bridge (Task 5) ✓ *(source compiles; musl artifact deferred)*; carrier (Task 6) ✓ *(WS loop unverified — no live sprite)*; mux+splice (Task 7) ✓; provider (Task 8) ✓; security posture is structural (loopback-only listeners, token-gated endpoints, outbound-only backend) ✓. **Deferred to live bring-up** (need `SPRITES_TOKEN` + a built musl bridge + a reachable backend): the exec/fs live probe, the musl binary in `priv/`, the WS-loop + end-to-end runtime test, and **mid-session carrier reconnect-on-drop** (initial-connect retry IS implemented). The spec's explicit `WINDOW` flow control is **consciously simplified** to bounded-channel backpressure for v1 — recorded in the spec decisions log.
```
