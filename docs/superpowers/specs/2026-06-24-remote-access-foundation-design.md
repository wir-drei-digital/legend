...
**Soundness constraint — no localhost-collapsing proxy in front.** If a TLS-terminating reverse proxy (Tailscale *Serve*, nginx) forwards to `127.0.0.1`, Phoenix sees *every* remote user as loopback and waves them through — a total auth bypass. Therefore, when remote access is enabled, **Phoenix binds the interface directly, with no fronting proxy** (Phase 2a binds `0.0.0.0`; TLS deferred — see Reachability), so a remote peer arrives with its real (non-loopback) source IP and "loopback = local" stays true. The peer IP comes from the socket/conn `peer_data`, never from a spoofable forwarded header (no trusted proxy exists in this topology).
