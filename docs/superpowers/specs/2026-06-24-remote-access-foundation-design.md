...
- **Bind the *specific* mesh interface, not `0.0.0.0`.** The safe default scopes reachability to the mesh address (e.g. the tailnet IP); binding all interfaces would expose the instance on every network the machine is on (LAN, any public IP) — only the auth rule would stand between it and the open internet. The interface is selected from the setting; the plan pins how it's resolved.
