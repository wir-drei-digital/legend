...
**Capability gating.** The toggle shows only when `transports` has >1 entry. ACP resume attempts `session/load` only if the agent advertised the `loadSession` capability at `initialize` (discovered at runtime — authoritative), else degrades to a fresh ACP session (mirroring how non-`resumable` terminal harnesses degrade today). Use `session/load` specifically (it replays history to the client); a bare resume restores only internal SDK state without replay. The gate is enforced in `Acp.Connection.handle_response(:initialize)` (`launch.mode == :load and load?`); without it a `:load` launch sends `session/load` unconditionally and a non-load-capable adapter fails the handshake with `-32601 "Method not found"`.

> **Observed (2026-06-22):** `claude-code-acp@0.13.x` (SDK `@agentclientprotocol/sdk@0.13`) **dropped `loadSession`** — it advertises `agentCapabilities.sessionCapabilities.resume` and implements `session/resume` instead (which resumes the conversation **without** replaying history, so it would not repopulate the rich timeline anyway). So in practice a Claude Code terminal→rich switch now **degrades to a fresh `session/new`**: the switch succeeds but the pre-switch transcript is not replayed into the rich view. Restoring true cross-transport replay needs an adapter that re-implements `loadSession`, or new client work to drive `session/resume` + fetch history.
...
- `claude-code-acp` (`@zed-industries/claude-code-acp`) — Claude Code ACP adapter; shared `~/.claude/projects/<encoded-cwd>/session-<uuid>.jsonl` store. **Note:** at the time of writing it supported `session/load` (replays JSONL history); as of `0.13.x` it **no longer implements `loadSession`** — it advertises `sessionCapabilities.resume` / `session/resume` (no history replay). See the "Observed (2026-06-22)" note under *Capability gating*.
