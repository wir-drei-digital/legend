...
- **Per-harness MCP registration.** Claude Code gets the server per launch (`--mcp-config` inline JSON + `--allowed-tools mcp__legend`). Hermes has no per-launch MCP flag — registration is a **one-time operator step**: an `mcp_servers.legend` entry in `~/.hermes/config.yaml` with `url: ${LEGEND_MCP_URL}` and `Authorization: Bearer ${LEGEND_SESSION_TOKEN}` placeholders. Hermes interpolates `${VAR}` from the process environment, and Legend injects exactly those vars into every spawned session, so the static entry resolves to the right endpoint + per-session identity. Standalone (non-Legend) Hermes runs leave the placeholders unresolved and that server just fails to connect — a harmless logged warning. Without this entry, Hermes sessions run fine but can't see the signal-bus tools (the documented terminal-fallback posture).
