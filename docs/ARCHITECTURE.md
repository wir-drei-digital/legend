...
- **Space model (`workspace.svelte.ts`).** A *Space* is `{ id, name, auto?, rail?, side?, layout: TileLayout }`. The store seeds two: the **auto Sessions space** (`auto: 'sessions'`) and the **Library space** (`rail/side: 'library'`); users create custom spaces (grid-only). Manual tiles carry a `{ kind, params }` binding (`#bindings`, keyed by minted `tile-N` id); the Sessions space is special — its **tile ids *are* session ids** and its bindings are *derived* (`binding()` returns `{kind:'session', params:{sessionId:id}}`). `openSurface(kind, params)` is the single entry point: it routes off the auto space (a `session` promotes onto the bench; a `file` switches to Library; anything else lands in a custom space) and dedupes via `key`.
- **Relocated Sessions watch-set.** The session-specific semantics the windowing core deliberately omits — the tile **cap** (6), the user-**dismissed** set, and **reconcile** against live sessions — live in `sessions-layout.svelte.ts` (`SessionsLayout` wrapping a `TileLayout`). `LegendShell` runs a `$effect` that reconciles the watch-set against the ranked live-session candidate list.
- **`SessionPane` decoupled.** The session view takes `{ session, grab?, layout, onClose }` (props, not store reads), so a session renders in **any** space, framed by whatever surface hosts it — `SessionSurface` in the grid, the bench in the Sessions space. The decouple is what lets sessions be just another surface.
- **Space-frame composition (`LegendShell`).** Three frames around a `TileGrid`, chosen by the active space's shape: auto Sessions renders `SessionBench` + grid **directly** (parity with the pre-tiling shell); Library wraps the grid in `WorkbenchLayout` (rail = `LibraryRail`, side = `LibrarySide`); custom spaces are the bare grid. Each frame supplies its own `empty` snippet.
...
Accepted caveats:
- **The auto Sessions space is sessions-only.** Reconcile rebuilds its columns from live sessions, so a foreign tile dropped in would be clobbered — surface mixing happens in Library/custom spaces, not the auto space. Its snapshot is a marker only (empty layout/bindings); it repopulates from live sessions via the reconcile effect on hydrate.
